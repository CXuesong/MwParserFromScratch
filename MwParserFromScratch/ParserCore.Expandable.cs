...
partial class ParserCore
{
    ...
    private ArgumentReference ParseArgumentReference()
    {
        ...
        if (ConsumeToken(@"\}\}\}") == null)
            return ParseFailed<ArgumentReference>();
        return ParseSuccessful(new ArgumentReference { Name = name, DefaultValue = defaultValue });
    }
    ...
    private TemplateArgument ParseTemplateArgument()
    {
        ...
        if (ConsumeToken(@"=") != null)
        {
            ...
            return ParseSuccessful(new TemplateArgument { Name = a, Value = value });
        }
        return ParseSuccessful(new TemplateArgument { Value = a });
    }
    ...
    private TagNode ParseTag()
    {
        ...
        TagNode node = options.ParserTagsSet.Contains(tagName)
    ...
    private bool ParseUntilClosingTag(TagNode tag)
    {
        ...
        {
            matcher = closingTagMatcherCache.GetValueOrDefault(normalizedTagName);
            if (matcher == null)
            {
